...
{
    public partial class deleteClinicForm : Form
    {
        Timer t = new Timer();
        string user;
        string userID;
        ...
        private void deleteClinicForm_Load(object sender, EventArgs e)
        {
        ...
        {
            if (this.clinicIDInput.Text == "" && this.clinicNameInput.Text == "")
            {
            ...
            else
            {
                try
                {
                    ...
                    {
                        string Conn = "datasource=localhost;port=3306;username=root;password=;database=medisupply;sslMode=none";
                        string Query = "DELETE FROM clinic WHERE clinicID = @clinicID";
                        MySqlConnection MyConn = new MySqlConnection(Conn);
                        MySqlCommand cmd = new MySqlCommand(Query, MyConn);
                        cmd.Parameters.AddWithValue("@clinicID", this.clinicIDInput.Text);
                        MyConn.Open();
