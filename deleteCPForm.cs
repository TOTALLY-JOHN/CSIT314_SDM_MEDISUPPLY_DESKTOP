...
{
    public partial class deleteCPForm : Form
    {
        Timer t = new Timer();
        string user;
        string userID;
        ...
        private void deleteCPForm_Load(object sender, EventArgs e)
        {
        ...
        {
            if (this.cpIdInput.Text == "" && this.cpNameInput.Text == "")
            {
            ...
            else
            {
                try
                {
                    ...
                    {
                        string Conn = "datasource=localhost;port=3306;username=root;password=;database=medisupply;sslMode=none";
                        string Query = "DELETE FROM contact_person WHERE cpID = @cpID";
                        MySqlConnection MyConn = new MySqlConnection(Conn);
                        MySqlCommand cmd = new MySqlCommand(Query, MyConn);
                        cmd.Parameters.AddWithValue("@cpID", this.cpIdInput.Text);
                        MyConn.Open();
